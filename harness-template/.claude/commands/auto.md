...
```
- 결과가 `{{DEV_BRANCH}}`가 아니면 → **즉시 에스컬레이션** (이후 단계 금지)
...
`BUGS > 0` → `BUG-N` 파싱 → 그룹화 → 버그수정자(들) 병렬 → 전원 완료 후 `{{BUILD_CMD}}` 1회 → active-lessons 추가 → `local_loop++` → 3.1로.
...
브랜치: {{DEV_BRANCH}}
```
`{{DEPLOY_DEV_CMD}}` 실행 + curl 헬스체크. `RESULT: FAILURE` → 에스컬레이션. SUCCESS → Phase 5.
