---
allowed-tools: Bash({{BUILD_CMD}}:*), Bash({{DEPLOY_DEV_CMD}}:*), Bash(git add:*), Bash(git status:*), Bash(git commit:*), Bash(git push:*), Bash(git diff:*), Bash(git log:*), Bash(curl:*), mcp__playwright__browser_navigate, mcp__playwright__browser_fill_form, mcp__playwright__browser_snapshot, mcp__playwright__browser_click, mcp__playwright__browser_take_screenshot, mcp__playwright__browser_wait_for, mcp__playwright__browser_console_messages
...
**테스트 계정:** URL `http://localhost:{{PREVIEW_PORT}}` · {{LOCAL_TEST_ACCOUNT}}
...
- **`{{DEV_BRANCH}}` 브랜치**: `{{DEPLOY_DEV_CMD}}`
- **`{{MAIN_BRANCH}}` 브랜치**: `{{DEPLOY_PROD_CMD}}`
...
**5-1. curl 헬스체크 (3회)**
```bash
curl -s -o /dev/null -w "%{http_code}" {{DEV_URL}}/login
```
- 3회 모두 200 확인 (cold start 포함)

**5-2. 브라우저 MCP Dev 테스트**
1. `browser_navigate` → `{{DEV_URL}}` 로그인 페이지
2. `browser_fill_form` 테스트 계정 로그인 ({{TEST_ACCOUNT}})
3. `browser_snapshot` 대시보드 정상 로딩 확인
4. 주요 페이지 네비게이션 테스트 + 각 페이지 `browser_snapshot`
5. 실패 시 `browser_take_screenshot` + 에러 보고

> **{{MAIN_BRANCH}} 브랜치인 경우**: Dev 검증 후 Production URL({{PROD_URL}})도 안정화 대기 후 동일 검증.
