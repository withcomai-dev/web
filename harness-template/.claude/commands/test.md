---
allowed-tools: Bash({{BUILD_CMD}}:*), Bash({{PREVIEW_CMD}}:*), Bash(curl:*), Bash(ls:*), mcp__playwright__browser_navigate, mcp__playwright__browser_fill_form, mcp__playwright__browser_snapshot, mcp__playwright__browser_click, mcp__playwright__browser_take_screenshot, mcp__playwright__browser_wait_for, mcp__playwright__browser_console_messages, mcp__playwright__browser_type, mcp__playwright__browser_press_key
...
Port {{PREVIEW_PORT}}에서 프리뷰가 시작되는지 확인.
...
**테스트 URL**: `http://localhost:{{PREVIEW_PORT}}`
**테스트 계정**: {{LOCAL_TEST_ACCOUNT}}
...
🧪 테스트 완료
- 빌드: 성공/실패
- 프리뷰: localhost:{{PREVIEW_PORT}} — 정상/실패
- 테스트 범위: {테스트한 기능 목록}
