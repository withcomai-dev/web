...
> {{RUNTIME}} 환경에서 절대 하면 안 되는 것들. 프로젝트별로 채운다.
...
## 배포 실패 대응 ({{DEPLOY_FAILURE_CODE}})

1. 경쟁 배포 프로세스 kill
2. 런타임 제약 위반 여부 확인 (위 §런타임 핵심 제약)
3. 재배포
4. 여전히 실패 → 로그 수집 (`{{LOG_CMD}}`) → 원인 격리
...
```bash
# 1. 직전 정상 커밋으로 즉시 롤백
{{ROLLBACK_CMD}}      # 예: git revert HEAD --no-edit && npm run deploy

# 2. 헬스체크
curl -s -o /dev/null -w "%{http_code}" {{PROD_URL}}/login   # 200 확인
...
- **자동 배포 가드** — `{{MAIN_BRANCH}}` 브랜치 커밋만 프로덕션 자동 배포 (post-commit 훅)
- **머지 가드** — `/tmp/{{PROJECT_SLUG}}-allow-merge` sentinel 없이는 main push/머지 차단 (pre-push/pre-merge-commit 훅)
